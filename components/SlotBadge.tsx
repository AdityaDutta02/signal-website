type Variant = "nav" | "inline" | "stamp";

export function SlotBadge({ variant = "inline", className = "" }: { variant?: Variant; className?: string }) {
  if (variant === "nav") {
    return (
      <div
        className={`hidden lg:flex items-center gap-2 px-3 py-1 border-2 border-line bg-bg font-mono text-[9px] font-bold tracking-widest uppercase ${className}`}
        title="Two build slots remaining for Q1 2026"
      >
        <span className="relative flex w-1.5 h-1.5">
          <span className="absolute inset-0 bg-pink anim-blink rounded-none" />
        </span>
